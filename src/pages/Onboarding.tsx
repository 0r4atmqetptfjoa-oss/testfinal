...
export default function Onboarding(){
  ...
  return (
    <div className="min-h-screen bg-black text-gray-200 px-4 pt-6 pb-10 max-w-xl mx-auto relative">
      {step>0 && (<button onClick={back} className="absolute top-3 left-3 z-40 rounded-full border border-ui bg-black/70 px-3 py-1 text-xs">Înapoi</button>)}
      ...
      {step===2 && (<div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {["Infanterie","Tancuri","Vânători de munte","Artilerie","Geniu","Comunicații","Logistică","Medical"].map(a=>(
            <button key={a} onClick={()=> setArma(a as any)} className={`rounded-2xl border p-4 text-left bg-card ${arma===a?"border-accent":"border-ui"}`}><div className="font-semibold">{a}</div><div className="text-xs text-gray-500">Curriculum dedicat</div></button>
          ))}
        </div>
        <div className="flex justify-between"><button onClick={back} className="px-4 py-2 rounded-xl border border-ui bg-card">Înapoi</button><button onClick={next} disabled={!arma} className={`px-4 py-2 rounded-xl btn btn-primary ${!arma?"opacity-40":""}`}>Continuă</button></div>
      </div>)}
      {step===3 && (<div className="space-y-4"><div className="rounded-2xl border border-ui bg-card p-4"><div className="text-sm">Gata! Profil creat. Poți schimba oricând din butonul „Schimbă profil”.</div></div><div className="flex justify-between"><button onClick={back} className="px-4 py-2 rounded-xl border border-ui bg-card">Înapoi</button><button onClick={finish} className="px-4 py-2 rounded-xl btn btn-primary">Finalizează</button></div></div>)}
    </div>
  );
}
