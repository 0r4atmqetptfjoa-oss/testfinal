...
// Toast helper to trigger global notifications
...
export default function ExamPlayer({ items, seconds=300, onFinish }: { items: ExamItem[]; seconds?: number; onFinish?: (score: number)=>void }){
  ...
  // Local toast state removed; we use react-hot-toast to display notifications
  const [done, setDone] = useState(false);
  const startedAt = useRef<number>(Date.now());

  useEffect(()=>{
    function onOffline(){ toast("Mod offline: continuă, răspunsurile sunt salvate local."); }
  ...
  function showXP(txt: string){ toast.success(txt); }
