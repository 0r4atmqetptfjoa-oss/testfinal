  ...
  return (
    <main className="min-h-screen bg-black text-gray-200 p-4 max-w-2xl mx-auto pb-28">
      <div className="space-y-3">
        {msgs.map(m => (
          <div key={m.id} className={"w-full flex "+(m.user==="me" ? "justify-end" : "justify-start")}>
            <div className={(m.user==="me" ? "bg-violet-600 text-white" : "bg-gray-900 text-gray-200 border border-gray-800") + " max-w-[85%] rounded-2xl px-3 py-2 whitespace-pre-wrap"}>
              {m.text}
            </div>
          </div>
        ))}
      ...
      <div className="fixed left-0 right-0 bottom-16 mx-auto max-w-2xl p-3">
        <div className="rounded-2xl border border-gray-800 bg-gray-900 flex gap-2 px-3 py-2">
          <input value={q} onChange={e=> setQ(e.target.value)} placeholder="Scrie o întrebare..." className="bg-transparent outline-none flex-1" />
