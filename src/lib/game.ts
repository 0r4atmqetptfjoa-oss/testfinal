...
export function loadGame(): GameState {
  ...
  saveGame(init); return init;
}
export function saveGame(s: GameState){ try{ localStorage.setItem(KEY, JSON.stringify(s)); }catch{} }
export function addActivityXP(s: GameState, amount: number, dateISO = todayISO()){
  if (!s.activity[dateISO]) s.activity[dateISO] = 0; s.activity[dateISO] += amount;
}
...
export function awardXP(amount: number, reason: string = "generic"){
  ...
  s.xp += amount; addActivityXP(s, amount, today); touchStreak(s, new Date());
  const newly = checkBadges(s, reason); saveGame(s);
  return { state: s, newlyUnlocked: newly };
}
export function levelFromXP(xp: number){
  const lvl = Math.floor(Math.sqrt(Math.max(0, xp)/100))+1;
  const start = 100*(lvl-1)*(lvl-1); const next = 100*lvl*lvl;
  const cur = xp - start; const need = next - start; const pct = need>0 ? Math.max(0, Math.min(1, cur/need)) : 1;
  return { level: lvl, start, next, cur, need, pct };
}
export function last7DaysActivity(): { day: string; xp: number }[] {
  const s = loadGame(); const days: { day: string; xp: number }[] = [];
  for (let i=6;i>=0;i--){ const d = new Date(); d.setDate(d.getDate()-i); const k = d.toISOString().slice(0,10); days.push({ day: k, xp: s.activity[k] || 0 }); }
  return days;
}
export function last30DaysActivity(): { date: string; xp: number }[] {
  const s = loadGame(); const out: { date: string; xp: number }[] = [];
  for (let i=29;i>=0;i--){ const d = new Date(); d.setDate(d.getDate()-i); const k = d.toISOString().slice(0,10); out.push({ date: k, xp: s.activity[k] || 0 }); }
  return out;
}
...
export function haptic(pattern: "light"|"success"|"error"|"impact" = "light"){
  try{
    ...
    const m = { light: [10], success: [15,30,15], error: [30,60,30], impact: [20] } as Record<string, number[]>;
