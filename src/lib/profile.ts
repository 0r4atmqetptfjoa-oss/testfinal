...
export function branchSlug(branch?: Branch|null){ const s=(branch || getProfile().branch || "").toString().toLowerCase(); return s.replace(/[ăâîșşţț]/g,"a").replace(/ș|ş/g,"s").replace(/ț|ţ/g,"t").replace(/[^a-z0-9 ]/g,"").trim().replace(/\s+/g,"-"); }
