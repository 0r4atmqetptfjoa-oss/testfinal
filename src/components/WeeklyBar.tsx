...
export default function WeeklyBar({ values }: { values: number[] }){
  ...
  return (
    <div className="flex items-end justify-between gap-2 select-none">
      {values.map((v, i)=>{
        const h = Math.round((v / max) * 66);
        return (
          <div key={i} className="flex flex-col items-center gap-1">
            <div className="w-6 rounded bg-cyan-400/15 border border-cyan-400/40 overflow-hidden" style={{ height: 68 }}>
